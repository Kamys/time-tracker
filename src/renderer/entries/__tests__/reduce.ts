...
describe('Update entity', () => {
  ...
  it('Update product move in start list', () => {
    const newCost = 780
    // @ts-ignore
    const action = ActionsEntries.change.REQUEST({
      id: 3,
      entityName: 'products',
      cost: newCost,
    })
    // @ts-ignore
    expect(reduce(defaultEntriesState, action).products[0].cost).toEqual(
      newCost,
    )
  })
  ...
  it('Can change product name', () => {
    ...
    // @ts-ignore
    expect(reduce(defaultEntriesState, action).products[0].name).toEqual(
      newName,
    )
  })
})
...
describe('Delete entity', () => {
  it('Can delete user', () => {
    // @ts-ignore
    const action = ActionsEntries.remove.REQUEST({
      entityId: 1,
      entityName: 'users',
    })
    // @ts-ignore
    expect(reduce(defaultEntriesState, action)).toEqual({
      ...defaultEntriesState,
      users: [defaultEntriesState.users[0], defaultEntriesState.users[2]],
    })
  })
